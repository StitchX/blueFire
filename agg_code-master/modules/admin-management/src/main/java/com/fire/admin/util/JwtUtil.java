...
public class JwtUtil {
    ...
    private static final String SUPERVIASION_BUREAU = "supervisionId";

    private static final String SUPERVIASION_BUREAU_IDS = "supervisionIds";
    /**
     * 创建时间
     */
    ...
    public static String generateToken(PreSecurityUser userDetail) {
        ...
        claims.put(SUPERVIASION_BUREAU, userDetail.getSupervisionId());
        claims.put(SUPERVIASION_BUREAU_IDS, userDetail.getSupervisionIds());
    ...
    public PreSecurityUser getUserFromToken(HttpServletRequest request) {
        ...
        if (StrUtil.isNotEmpty(token)) {
            ...
            Long supervisionId = ObjectUtil.isNotEmpty(claims.get(SUPERVIASION_BUREAU)) ? Long.parseLong(claims.get(SUPERVIASION_BUREAU).toString()) : null;
            JSONArray array = JSONUtil.parseArray(claims.get(SUPERVIASION_BUREAU_IDS));
            Set<Long> Ids = new HashSet<>(array.toList(Long.class));
            ...
            if (validateToken(token, username)) {
                // 未把密码放到jwt
                return new PreSecurityUser(userId, username, "", authorities, supervisionId, Ids);
            }
        }
        return null;
    }
