    ...
    <div>
      <el-row style="display: flex;flex-direction:row;flex-wrap:wrap">
        <el-col :span="7" :lg="9" :xs="24" :sm="12" :md="12">
          <notice :data="Content" :titel="ruleForm.noticeTitle" :type="noticeType" :date="systemTime" :issuing-agency="ruleForm.issuingAgency" :height="480" :show="ruleForm.readFlag"/>
        </el-col>
        <el-col :span="15" :lg="15" :xs="24" :sm="12" :md="12" class="scrollBar">
          <div class="grid-content bg-purple1">
            <span class="grid-text">编辑通知</span>
            <el-divider />
...
import { treeList, ossUp, noticeInfo, quyuList, noticeAdd, noticeUpdate } from './../../api/notice'
...
export default {
    ...
    urlId() {
      ...
      if (arry) {
        ...
        getnoticeId(data).then((res) => {
          ...
          this.ruleForm = data
          this.fillerList(data)
          console.log(data.noticeScope)
          const code = data.noticeScope
          code.map((res) => {
            // 如果区域和行业被选中
            if (res == 3) {
              // 展示出来
            ...
            if (res == 4) {
              this.showjingzhun = true
            }
          })
          // 如果选择行业为ture
          if (data.categorys) {
            const info = data.categorys.map((res) => {
              console.log(res.categoryId)

              return res.categoryId
            })
            data.categoryIds = info
          }
          //   如果选择区域
          if (data.noticeChooseSupervisionBureau) {
            const arry = []
            data.noticeChooseSupervisionBureau.map((res, i) => {
              if (res.flag) {
                console.log(res)
                arry.push(res.supervisionId)
              }
            })
            this.ruleForm.supervisionIds = arry
          }
          // categoryIds
          // 副文本内容
          this.editor.txt.html(data.content)
          const dateFile = data.adjunctFile.map((res) => {
            return { name: res.fileName, url: res.ossFilePath }
          })
          console.log(dateFile)
          this.fileList = dateFile
          //    this.fileList = data.adjunctFile
        })
      } else {
        this.ruleForm = {
          noticeScope: [],
  ...
  if (fileSuffix=="mp4" && !isLt2M) {
    ...
    handRemove(file, fileList) {
      console.log(file)
      const data = this.ruleForm.adjunctFile.map((res,index) => {
        console.log(res,index)
        if(file.name == res.fileName){
          this.ruleForm.adjunctFile.splice(index,1)
        }
      })
