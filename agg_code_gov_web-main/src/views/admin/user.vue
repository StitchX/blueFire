      ...
      <el-col :xs="24" :sm="24" :md="20" :lg="18" :xl="18">
        ...
        <el-table v-loading="loading" :data="tableData" border style="width: 100%">
          ...
          <el-table-column label="监管所" align="center">
            <template slot-scope="scope">
              <div>{{ scope.row.supervisionName }}</div>
            </template>
          </el-table-column>
        ...
        <el-dialog :title="operation?'新增用户':'编辑用户'" :visible.sync="dialogFormVisible" center>
          <el-form ref="dataForm" :model="dataForm" :rules="rules2" label-width="80px" size="small" label-position="right">
            ...
            <el-form-item label="监管所" :label-width="formLabelWidth">
              <popup-tree-input
                :data="deptData"
                :props="deptTreeProps"
                :prop="dataForm.supervisionName"
                :node-key="dataForm.supervisionId"
          ...
          <div slot="footer" class="dialog-footer">
            ...
            <el-button type="primary" :loading="butonLoding" @click="submitForm">确 定</el-button>
          </div>
        </el-dialog>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    deptTreeCurrentChangeHandle(data) {
      console.log(data)
      // 不加入toString会报错 ： type check failed for prop "nodeKey". Expected String, got Number.
      this.dataForm.supervisionId = data.supervisionId.toString()
      this.dataForm.supervisionName = data.supervisionName
    },
    // 加载用户列表
    adminList: function() {
      this.loading = true
      ...
      params.append('supervisionId', this.supervisionId)
    ...
    handleNodeClick(data) {
      console.log(data)
      this.supervisionId = data.supervisionId === '' ? '' : data.supervisionId
      this.adminList()
    },
    submitForm: function() {
      const userRoles = []
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          if (!this.operation) {
            ...
            }).catch(error => this.butonLoding = false)
          }
        }
      })
    },
